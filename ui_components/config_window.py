...
class ConfigWindow(ctk.CTk):
    def __init__(self):
        ...
        # 타이틀 (중앙 정렬 유지 혹은 왼쪽 정렬 선택 가능 - 여기서는 타이틀만 중앙)
        self.label_title = ctk.CTkLabel(self.scrollable_frame, text="수집 프로그램 설정", font=self.title_font)
        self.label_title.pack(pady=(30, 30))

        # --- 1. Gemini API Key ---
        self._create_label("Gemini API Key")
        self.entry_gemini = ctk.CTkEntry(self.scrollable_frame, width=580, height=45, font=self.entry_font, show="*")
        self.entry_gemini.pack(pady=(5, 15), anchor="w", padx=85) # anchor="w" 추가
    ...
    def _create_label(self, text):
        """레이블 생성을 위한 헬퍼 함수"""
        lbl = ctk.CTkLabel(self.scrollable_frame, text=text, font=self.label_font)
